...
def search_guides():
    if "--help" in input_args:
        sys.stderr.write(
            "Search guides for CRISPR/Cas9 genome editing based on the given "
            "PAM sequence, genome sequence, BED file, maximum number of "
            "mismatches, and output file name\n"
        )
        sys.stderr.write("Options:\n")
        sys.stderr.write(
            "\t--fasta, FASTA file contaning either a single chromosome or a "
            "target sequence extracted from genome\n"
        )
        sys.stderr.write("\t--pam, PAM file\n")
        sys.stderr.write(
            "\t--coordinates, Coordinate file in BED format (Please Note: only "
            "one chromosome allowed at a time [OPTIONAL]\n"
        )
        sys.stderr.write(
            "\t--mm, Maximum allowed mismatches in the PAM sequence [OPTIONAL, default 0]\n"
        )
        sys.stderr.write(
            "\t--output-prefix, Output file prefix: the generated files include "
            "the guides and a detailed report [OPTIONAL, default guides_out]\n"
        )
    # parse input args
    if "--fasta" not in input_args:
        sys.stderr.write("Please input some parameter for flag --fasta\n")
        sys.exit(1)
    else:
        try:
            fasta = input_args[input_args.index("--fasta") + 1]
        except IndexError as e:
            sys.stderr.write("Please input some parameter for flag --fasta\n")
            sys.exit(1)
    if "--pam" not in input_args:
        sys.stderr.write("Please input some parameter for flag --pam\n")
        sys.exit(1)
    else:
        try:
            pam = input_args[input_args.index("--pam") + 1]
        except IndexError as e:
            sys.stderr.write("Please input some parameter for flag --pam\n")
            sys.exit(1)
    if "--coordinates" in input_args:
        try:
            coordinates = input_args[input_args.index("--coordinates") + 1]
        except IndexError as e:
            sys.stderr.write("Please input some parameter for flag --coordinates\n")
            sys.exit(1)
    else:
        coordinates = None
    if "--mm" in input_args:
        try:
            ...
            sys.stderr.write("Please input some parameter for flag --mm\n")
            sys.exit(1)
    else:
        mm = None
    if "--output-prefix" in input_args:
        try:
            outputpref = input_args[input_args.index("--output-prefix") + 1]
        except IndexError as e:
            sys.stderr.write("Please input some parameter for flag --output-prefix\n")
            sys.exit(1)
    else:
        outputpref = None
    # run guides search
    searcher = os.path.join(script_path, "design-guides/design_guides.py")
    cmd = f"python {searcher} --fasta {fasta} --pam {pam}"
    for flag, param in {
        "--coordinates": coordinates,
        "--mm": mm,
        "--output-prefix": outputpref,
    }.items():
    ...
    try:
        ...
        raise subprocess.SubprocessError("Guides search failed!") from e
...
def callHelp():
    print(
        ...
        "\ncrisprme.py search-guides FUNCTION TO SEARCH POTENTIAL GUIDES IN A SEQUENCE OR A SET OF COORDINATES USING A SPECIFIED PAM SEQUENCE"
