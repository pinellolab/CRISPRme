        ...
        try:
            ...
            if os.path.isdir(outputfolder):  # check whether the folder is present or not
                if is_folder_empty(outputfolder):  # if present check if not empty
                    sys.stdout.write(f"\nOutput folder {outputfolder} not empty!\nSelect another output folder for the current CRISPRme run.\nIf the previous run using the following folder threw an error, please delete {outputfolder} before running a new CRISPRme search.\n\n")
                    exit(1)
