    ...
    try:
        if verbosity > 2:
            ...
            code = subprocess.call(
                run, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT
            )
        if code != 0:
            exception_handler(
                CRISPRitzError,
                f"An error occurred while running {CRISPRITZ} {command}",
                debug,
            )
    except Exception:
        exception_handler(
...
def crispritz_add_variants(
    vcf_folder: str, genome_folder: str, verbosity: int, debug: bool
) -> None:
    ...
    if not isinstance(vcf_folder, str):
        exception_handler(
            TypeError,
            f"Expected {str.__name__}, got {type(vcf_folder).__name__}",
            debug,
        )
    ...
    run_crispritz(
        crispritz_run, ADDVARIANTS, verbosity, debug
    )  # run crispritz add-variants


def crispritz_index_genome(
    genome: str,
    genome_folder: str,
    pam: str,
    bulges: int,
