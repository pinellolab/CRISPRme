...
MMS_TYPE_REPLACEMENT = {'0': "match", '-1': "indel", '1': "wobble",
                        '2': "RR transition", '3': "YY transition", '4': "transversion"}
...
def count_mismatches(aligned_seq1, aligned_seq2):
    ...
    for i in range(ending):
        cnt += int(aligned_seq1[i] != aligned_seq2[i]
                   and aligned_seq1[i] != "-" and aligned_seq2[i] != "-")
    return cnt
...
def get_DNAshape_features(dna_seq):
    ...
    for i in range(2, len(dna_seq)-2):
        ...
        current_heptamer = re.sub("N", random.choice(
            ["A", "C", "G", "T"]), current_heptamer)
...
def get_features(full_dna_seq, aligned_sgRNA, aligned_offtarget, pa_score):
    ...
    nucleotides_down_pam = [agct2numerals(c)
                            for c in full_dna_seq[-3:]] + [0, 0]

    # geometry features: dna_enthalpy
    extended_dna_enthalpy = sum(
        [DNA_PAIRS_THERMODYNAMICS[full_dna_seq[i-1:i+1]] for i in range(1, len(full_dna_seq))])
...
def align_sequences(sgRNA, genomic_extended):
    ...
    for i in [0, 6, 1, 5, 2, 4, 3]:  # because starting at 3 is the original - we'd prefer that
        ...
        (alnA, alnB, score) = PA_script.align_pair(seqA=sgRNA[:-3], seqB=current_dna, match_score=MATCH_SCORE,
                                                   mismatch_score=MISMATCH_PENALTY, gap_score=GAP_PENALTY, gaps_allowed=MAX_ALLOWED_GAPS)  # regular pa
...
def get_alignment_score(sgRNA, genomic_extended):
    ...
    for i in range(len(offtarget_seq)):
        score += two_chars_score(sgRNA[i], offtarget_seq[i],
