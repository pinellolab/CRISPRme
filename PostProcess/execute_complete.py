...
def pre_process():
    ...
    dummy_file.close()
    ##CREATE EMPTY FILE
    empty_file=open(f"{output_folder}/.empty.txt","w")
    empty_file.close()
    ##CREATE EMPTY DIR
...
def generate_dict(vcf_data):
    ...
    list_files=os.listdir(os.path.join(genomes_folder,"variants_genome","SNPs_genome"))
    for file in list_files:
        if ".json" in file:
            os.rename(os.path.join(genomes_folder,"variants_genome","SNPs_genome",file),os.path.join(dictionaries_folder,"dictionaries_"+vcf_name,file))
        if "log" in file:
            os.rename(os.path.join(genomes_folder,"variants_genome","SNPs_genome",file),os.path.join(dictionaries_folder,"log_indels_"+vcf_name,file))
