...
# The $mm variable defines the number of mismatches used for the search (default: 6),
# while $bDNA and $bRNA define the number of allowed DNA and RNA bulges (default: 2).
...
./analisi_indels_NNN.py \
    "$annotations" \
    "$jobid.total.cluster.txt" \
    "$jobid" \
    "$dictionaries" \
    "$pam_file" \
    "$mm" \
