...
def get_features(full_dna_seq, aligned_sgRNA, aligned_offtarget, pa_score):
    ...
    nucleotides_down_pam = [agct2numerals(c) for c in full_dna_seq[-3:]] + [
        0,
        0,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CRISTA, a tool for CRISPR Target Assessment"
    )
    parser.add_argument(
        "--sgseq", "-s", required=True, help="sgRNA seq of 20 bases (without PAM)"
    )
    parser.add_argument(
        "--genomic_seq",
        "-d",
        required=True,
        help="DNA target sequence with 3 additional bases at each end (total of 29 nucleotides)",
    )
    ...
    try:
        ...
        assert (
            sgRNA_seq_re is not None and len(sgRNA_seq_re.group()) == 20
        ), "sgRNA sequence must be 20-nt long sequence of ACGTU nucleotides"
        full_dna_seq_re = re.search("[acgtu]+", full_dna_seq, re.IGNORECASE)
        assert (
            full_dna_seq_re is not None and len(full_dna_seq_re.group()) == 29
        ), "genomic sequence must be 29-nt long sequence of ACGTU nucleotides"
    except:
    ...
    ### align_sequences
    aligned_sgRNA, aligned_offtarget, max_score = align_sequences(
        sgRNA=sgRNA_seq, genomic_extended=full_dna_seq
    )

    ### get features
    features = get_features(
        full_dna_seq=full_dna_seq,
