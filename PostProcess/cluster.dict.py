...
process = subprocess.Popen(
    ["wc", "-l", sys.argv[1]], stdout=subprocess.PIPE, stderr=subprocess.PIPE
)
...
if "orderChr" in sys.argv[:]:
    ...
    subprocess.run(
        [
            f"sort -T {out_directory} -k4,4 -k6,6 -k15,15 -k7,7 "
            + sys.argv[1]
            + " > "
            + result_name
            + ".tmp_sort.txt"
    ...
    with open(result_name + ".tmp_sort.txt") as targets, open(
        ...
        if "total" not in sys.argv[:]:
            ...
            result.write(
                "#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tCluster Position\tDirection\tMismatches\tBulge_Size\tTotal\tPAM_gen\tVar_uniq"
                + add_for_final
            )
        ...
        for line in targets:
            ...
            if (
                ...
                current_cluster.sort(
                    key=lambda x: (x[9], x[7], [alphabet.index(c) for c in x[2]])
...
if total_line > MAX_LIMIT:
    ...
    with open(sys.argv[5]) as guides:
        for guide in guides:
                    ...
                    if current_count > MAX_LIMIT:
                        ...
                        subprocess.run(
                            [
                                ...
                                + sys.argv[1]
                                + " "
                                + sys.argv[1].replace(".txt", ".cluster.txt")
                            ],
                            shell=True,
                        )
                        cluster_ok = False
                        ...
                        break
                    try:
                        guides_dict[line[1].replace("-", "")].append(
                            (
                                "\t".join(line[:2]),
                                line[2],
                                line[3],
                        ...
                        )  # [('type\tguide', 'target', 'chr', pos, clusterpos, 'dir', mm, bul, tot)]
                    except:
            ...
            if not cluster_only:
                print(
                    "Created 'Total' and 'Position Cluster' columns:",
                    time.time() - start,
                )
            else:
            ...
            with open(result_name, "a+") as result:
                if write_header:
                    if (
                        ...
                        if "addForFinal" in sys.argv[:]:
                            result.write(
                                "#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tDirection\tMismatches\tBulge_Size\tTotal\tPAM_gen\tGenome"
                                + add_for_final
                            )
                        else:
                            result.write(
                ...
                current_chr_pos = (
                    first_line[2] + str(first_line[4]) + first_line[5]
                ...
                total_list[-1].sort(
                    key=lambda x: (x[8], x[6], [alphabet.index(c) for c in x[1]])
                ...
                if addGuide:
                    if keep_columns:
                        ...
                        for cluster in total_list:
                            for target in cluster:
                                result.write(
                                    "\t".join(str(x) for x in target[:4])
                                    + "\t"
                                    + "\t".join(str(x) for x in target[5:8])
                                    + "\t"
                                    + target[9].strip()
                                    + "\t"
                                    + target[0].split("\t")[1].replace("-", "")
                        ...
                        if "addForFinal" in sys.argv[:]:
                            ...
                            for cluster in total_list:
                                for target in cluster:
                                    ...
                                    if (
                                        last_info[2] != "n"
                                    ):  # target has at least one sample
                                        last_info[1] = "ENR"
                                    else:
                                        last_info[1] = "REF"
                                    remove_indices = {
