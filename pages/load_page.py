...
def refresh_search(n: int, dir_name: str) -> Tuple:
    ...
    if dir_name.split('=')[-1] in job_data:
        job_data = [
            f for f in os.listdir(current_job_directory)
            if (
                not f.startswith(".") and
                os.path.isfile(os.path.join(current_job_directory, f))
            )
        ]
        ...
        if LOG_FILE in job_data:
            try:
                with open(
                    os.path.join(current_job_directory, LOG_FILE)
                ) as handle_log:
                    done = 0
                    ...
                    if variants:
                        if (
                            "Search Reference\tEnd" in current_log and 
                            "Search Variant\tEnd" in current_log and 
                            "Search INDELs\tEnd" in current_log
                        ):
                            ...
                            "Search Reference\tStart" in current_log or 
                            "Search Variant\tStart" in current_log
                        ):
                            search_status = html.P(
                    ...
                    if variants:
                        if (
                            "Post-analysis SNPs\tEnd" in current_log and 
                            "Post-analysis INDELs\tEnd" in current_log
                        ):
                    ...
                    if done == 7 or "Job\tDone" in current_log:
                        return (
                            ...
                            database_status, 
                            os.path.join(
                                URL, f"result?job={dir_name.split('=')[-1]}"
                            ), 
                            "", 
                            True
                        )
        ...
        # job has been queued
            ...
            return (
                ...
                "", 
                dbc.Alert(
                    "Job submitted. Current status: IN QUEUE", color="info"
                ), 
                True
            )
    # job data not found
                    ...
                    [
                        ...
                        html.Div(
                            html.P(
                                ...
                                id="job-link", 
                                style={
                                    "margin-top": "0.75rem", "font-size": "large"
                                }
                            ),
                            style={
                                "border-radius": "5px", 
