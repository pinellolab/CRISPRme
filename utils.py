...
def download_genome(chr: str, directory: str) -> None:
    ...
    if chr == "all":
        os.system(
            "curl -v https://hgdownload.soe.ucsc.edu/goldenPath/hg38/bigZips/hg38.chromFa.tar.gz > hg38.chromFa.tar.gz"
        )
        ...
        os.system(
            f"curl -v https://hgdownload2.soe.ucsc.edu/goldenPath/hg38/chromosomes/{chr}.fa.gz > {chr}.fa.gz"
        )
...
def download_vcf(chr: str, origin: str) -> None:
    ...
    if chr == "all" and origin == "1000G":
        ...
        for i in range(1, 23):
            os.system(
                f"curl -v ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/1000_genomes_project/release/20190312_biallelic_SNV_and_INDEL/ALL.chr${i}.shapeit2_integrated_snvindels_v2a_27022019.GRCh38.phased.vcf.gz > ALL.chr${i}.shapeit2_integrated_snvindels_v2a_27022019.GRCh38.phased.vcf.gz"
            )
            os.system(
        ...
        os.makedirs(f"VCFs/hg38_1000G", exist_ok=True)
        os.system(
            f"curl -v ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/1000_genomes_project/release/20190312_biallelic_SNV_and_INDEL/ALL.{chr}.shapeit2_integrated_snvindels_v2a_27022019.GRCh38.phased.vcf.gz > ALL.{chr}.shapeit2_integrated_snvindels_v2a_27022019.GRCh38.phased.vcf.gz"
        )
        os.system(
        ...
        for i in range(1, 23):
            os.system(
                f"curl -v ftp://ngs.sanger.ac.uk:21/production/hgdp/hgdp_wgs.20190516/hgdp_wgs.20190516.full.chr{i}.vcf.gz > hgdp_wgs.20190516.full.chr{i}.vcf.gz"
            )
            os.system(f"mv hgdp_wgs.20190516.full.chr{i}.vcf.gz VCFs/hg38_HGDP/")
    elif chr != "all" and origin == "HGDP":
        os.makedirs(f"VCFs/hg38_HGDP", exist_ok=True)
        os.system(
            f"curl -v ftp://ngs.sanger.ac.uk:21/production/hgdp/hgdp_wgs.20190516/hgdp_wgs.20190516.full.{chr}.vcf.gz > hgdp_wgs.20190516.full.{chr}.vcf.gz"
        )
...
def download_samplesID() -> None:
    os.system(
        "curl -v https://raw.githubusercontent.com/pinellolab/CRISPRme/test-function/download_data/hg38_1000G.samplesID.txt > hg38_1000G.samplesID.txt"
    )
    os.system(
        "curl -v https://raw.githubusercontent.com/pinellolab/CRISPRme/test-function/download_data/hg38_HGDP.samplesID.txt > hg38_HGDP.samplesID.txt"
    )
    os.system(
        "curl -v https://raw.githubusercontent.com/pinellolab/CRISPRme/test-function/download_data/hg38_gnomAD.samplesID.txt > hg38_gnomAD.samplesID.txt"
    )
...
def download_annotation() -> None:
    os.system(
        "curl -v https://github.com/pinellolab/CRISPRme/raw/test-function/download_data/gencode.protein_coding.tar.gz > gencode.protein_coding.tar.gz"
    )
    os.system("tar -xzf gencode.protein_coding.tar.gz")
    os.system("mv gencode.protein_coding.bed Annotations/")
    os.system(
        "curl -v https://github.com/pinellolab/CRISPRme/raw/test-function/download_data/encode+gencode.hg38.tar.gz > encode+gencode.hg38.tar.gz"
    )
