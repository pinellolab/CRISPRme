...
import os
MAX_LIMIT = 50000000
# For forcing IUPAC before reference target
...
process = subprocess.Popen(['wc', '-l', sys.argv[1]],
...
if 'orderChr' in sys.argv[:]:
    ...
    subprocess.run(['sort -T ' + dirname + ' -k4,4 -k6,6 -k15,15 -k7,7 ' + sys.argv[1] + ' > ' + result_name +
                    '.tmp_sort.txt'], shell=True)  # sort input file by chr, clusterpos, real guide, direction
    ...
    with open(result_name + '.tmp_sort.txt') as targets, open(result_name, 'w+') as result:
        ...
        if 'total' not in sys.argv[:]:
            if addGuide:
                if keep_columns:
                    result.write(
                        '#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tCluster Position\tDirection\tMismatches\tBulge_Size\tTotal\tReal Guide\n')
                else:
                    result.write(
                        '#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tDirection\tMismatches\tBulge_Size\tReal Guide\n')
            else:
                if keep_columns:
                    result.write(
                        '#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tCluster Position\tDirection\tMismatches\tBulge_Size\tTotal\n')
                else:
                    result.write(
                        '#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tDirection\tMismatches\tBulge_Size\n')
        else:
            result.write(
                '#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tCluster Position\tDirection\tMismatches\tBulge_Size\tTotal\tPAM_gen\tVar_uniq' + add_for_final)
        ...
        for line in targets:
            ...
            if line[3] + line[5] + line[6] + line[14] != current_chr_pos_dir_g:  # NEW CLUSTER FOUND
                ...
                # Sort by total, mismatches, and prioritize IUPAC characters
                current_cluster.sort(key=lambda x: (
                    x[9], x[7], [alphabet.index(c) for c in x[2]]))
...
if total_line > MAX_LIMIT:
    ...
    with open(sys.argv[5]) as guides:
        for guide in guides:
            ...
            with open(sys.argv[1]) as targets:
                for line in targets:
                    ...
                    if current_count > MAX_LIMIT:
                        print('The guide ' + guide + ' has more than ' +
                              str(MAX_LIMIT) + ' targets. Skipping...')
                        # with open('./guides_error.txt', 'a+') as guides_error:
                        #     guides_error.write(guide + '\n')
                        subprocess.run(
                            ['mv ' + sys.argv[1] + ' ' + sys.argv[1].replace('.txt', '.cluster.txt')], shell=True)
                        cluster_ok = False
                        del guides_dict[guide]
                        break
                    try:
                        guides_dict[line[1].replace('-', '')].append(('\t'.join(line[:2]), line[2], line[3], int(line[4]), int(line[5]), str(line[6]), int(
                            line[7]), int(line[8]), int(line[9]), '\t'.join(line[10:])))  # [('type\tguide', 'target', 'chr', pos, clusterpos, 'dir', mm, bul, tot)]
                    except:
                        guides_dict[line[1].replace('-', '')] = [('\t'.join(line[:2]), line[2], line[3], int(line[4]), int(
                            line[5]), str(line[6]), int(line[7]), int(line[8]), int(line[9]), '\t'.join(line[10:]))]
            if not cluster_ok:
                continue
            if not cluster_only:
                print('Created \'Total\' and \'Position Cluster\' columns:',
                      time.time() - start)
            else:
            ...
            for k in guides_dict.keys():
                # Order by chr_clusterpos
                guides_dict[k].sort(key=lambda x: (x[2], x[4]))
            ...
            with open(result_name, 'a+') as result:
                if write_header:
                    # TODO fix for offline release, praticamente se sto facendo cluster su total.txt metto l'header custom
                    if 'total' not in sys.argv[:]:
                        if addGuide:
                            if keep_columns:
                                result.write(
                        ...
                        if 'addForFinal' in sys.argv[:]:
                            result.write(
                                '#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tDirection\tMismatches\tBulge_Size\tTotal\tPAM_gen\tGenome' + add_for_final)
                        else:
                            result.write(
                                '#Bulge_type\tcrRNA\tDNA\tChromosome\tPosition\tCluster Position\tDirection\tMismatches\tBulge_Size\tTotal\tPAM_gen\tVar_uniq' + add_for_final)
                    write_header = False
                ...
                # chr clusterpos direction
                current_chr_pos = first_line[2] + \
                    str(first_line[4]) + first_line[5]
                ...
                for line in total_targets[1:]:
                    ...
                    if line[2] + str(line[4]) + line[5] != current_chr_pos:
                        ...
                        # Order cluster by total and mms, and prioritize targets with IUPAC
                        total_list[-1].sort(key=lambda x: (x[8],
                                                           x[6], [alphabet.index(c) for c in x[1]]))
                ...
                # Order last cluster by total and mms, and prioritize targets with IUPAC
                total_list[-1].sort(key=lambda x: (x[8],
                                                   x[6], [alphabet.index(c) for c in x[1]]))

                # Order all clusters by total of top1
                total_list.sort(key=lambda x: x[0][8])
                if 'orderChr' in sys.argv[:]:
                    # and then for chr, needed for sample annotation and summary by position
                    total_list.sort(key=lambda x: x[0][2])
                if addGuide:
                    if keep_columns:
                        for cluster in total_list:
                            for target in cluster:
                                result.write('\t'.join(str(x) for x in target).strip(
                                ) + '\t' + target[0].split('\t')[1].replace('-', '') + '\n')
                    else:
                        for cluster in total_list:
                            for target in cluster:
                                result.write('\t'.join(str(x) for x in target[:4]) + '\t' + '\t'.join(str(
                                    x) for x in target[5:8]) + '\t' + target[9].strip() + '\t' + target[0].split('\t')[1].replace('-', '') + '\n')
                    ...
                    if keep_columns:
                        if 'addForFinal' in sys.argv[:]:
                            ...
                            for cluster in total_list:
                                for target in cluster:
                                    # Contains from PAM creation to RealGuide
                                    last_info = target[-1].split('\t')
                                    if last_info[2] != 'n':  # target has at least one sample
                                        last_info[1] = 'ENR'
                                    else:
                                        last_info[1] = 'REF'
                                    # Remove cluster column and last column, but this one will get written through last_info
                                    remove_indices = {4, 9}
                                    result.write('\t'.join([str(x) for pos, x in enumerate(
                                        target) if pos not in remove_indices]).strip() + '\t' + '\t'.join(last_info).strip() + '\n')
                        else:
                            for cluster in total_list:
                                for target in cluster:
                                    result.write(
                                        '\t'.join([str(x) for x in target]).strip() + '\n')
                    else:
                        for cluster in total_list:
                            for target in cluster:
                                result.write('\t'.join(str(x) for x in target[:4]) + '\t' + '\t'.join(
                                    str(x) for x in target[5:8]) + '\t' + target[9].strip() + '\n')
            del guides_dict[guide]
            ok_guides.append(guide)
            print("Clustering runtime: %s seconds" %
    ...
    with open(result_name, 'w+') as result:
        ...
        for line in total_targets[1:]:
            ...
            if line[2] + str(line[4]) + line[5] != current_chr_pos:
                # total_list[-1].sort(key = lambda x: int(x[8]))
                # Order cluster by total and mms
                total_list[-1].sort(key=lambda x: (x[8],
                                                   x[6], [alphabet.index(c) for c in x[1]]))
                total_list.append([line])
                # current_chr_pos = line[3] + ' ' + line[9]
        ...
        # Order last cluster by total and mms
        total_list[-1].sort(key=lambda x: (x[8], x[6],
                                           [alphabet.index(c) for c in x[1]]))
        # Order all clusters by total of top1
        total_list.sort(key=lambda x: x[0][8])
        if 'orderChr' in sys.argv[:]:
            # and then for chr, needed for sample annotation and summary by position
